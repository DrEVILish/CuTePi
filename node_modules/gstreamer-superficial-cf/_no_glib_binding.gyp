	  ...
	  "conditions" : [
		["OS=='linux'", {
			...
			"libraries": [
				...
				'<!@(env PKG_CONFIG_SYSROOT_DIR=/home pkg-config gstreamer-1.0 --libs | sed s/-lglib[^\b]*//)',
				'<!@(pkg-config gstreamer-app-1.0 --libs | sed s/-lglib[^\b]*//)',
				'<!@(env PKG_CONFIG_SYSROOT_DIR=/home pkg-config gstreamer-app-1.0 --libs | sed s/-lglib[^\b]*//)',
				'<!@(pkg-config gstreamer-video-1.0 --libs | sed s/-lglib[^\b]*//)',
				'<!@(env PKG_CONFIG_SYSROOT_DIR=/home pkg-config gstreamer-video-1.0 --libs | sed s/-lglib[^\b]*//)'
			]
		}],
		["OS=='mac'", {
			"include_dirs": [
