...
const getFrameBufferResolution = () => {
    return new Promise((resolve, reject) => {
        exec('fbset', (error, stdout) => {
            if (error) {
                console.error(`Error retrieving framebuffer resolution: ${error}`);
                return reject(error); // Reject the promise with error
            }
            const widthMatch = stdout.match(/geometry\s+(\d+)\s+\d+/);
            const heightMatch = stdout.match(/geometry\s+\d+\s+(\d+)/);
            if (widthMatch && heightMatch) {
                ...
                console.error('Could not parse framebuffer resolution');
                reject(new Error('Failed to parse framebuffer resolution')); // Reject with error
            }
        });
    });
};
...
class GStreamerPlayer {
    ...
    async _loadToPipeline(filename) {
      ...
      const pipe = `
          ...
          filesrc location="${filePath}" ! decodebin ! videoconvert ! comp.
        `;
    ...
    // Function to crossfade to a new video
    async crossfadeToNewVideo(filename) {
      if (this.pipeline1) {
        ...
        this.pipeline1.stop(); // Stop the old pipeline
        this.pipeline1 = this.pipeline2; // Transfer reference
        this.pipeline2 = null;
      }
    };
